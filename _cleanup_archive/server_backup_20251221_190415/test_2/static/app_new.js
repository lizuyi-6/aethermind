    ...
    processMarkdown(markdown) {
        ...
        html = html.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (match, text, url) => {
            ...
            if (url.includes('/api/download/report/') || text.includes('下载报告') || text.includes('下载')) {
                return `<a href="#" onclick="event.preventDefault(); if(typeof app !== 'undefined' && app.handleDownload) { app.handleDownload(); } else { alert('请使用页面上的"导出报告"按钮下载PDF文件'); } return false;" class="download-link">${text}</a>`;
            }

...
# 备份
with open('/root/test_2/agent.py.bak5', 'w', encoding='utf-8') as f:
    f.write(content)
...
# 使用chr(10)代替换行符
NL = chr(10)
new1 = f'''        while continuation_count < max_continuations:
            # 【修复】硬性超时检测
...
new3 = f'''                else:
                    ...
                    if self._empty_content_count >= 2:
                        yield "{NL}[错误] 连续2次未获取内容，停止{NL}"
                        break
                    if len(full_content) >= 40000:
                        yield "{NL}[信息] 内容足够，停止{NL}"
                        break
                    continue'''

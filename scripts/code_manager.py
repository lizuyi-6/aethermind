...
class CodeManager:
    ...
    def save_codes(self, codes: Dict[str, int]) -> bool:
        ...
        with self._lock:
            try:
                data = {'codes': codes}
                codes_dir = os.path.dirname(self.codes_file) or '.'
                fd, tmp_path = tempfile.mkstemp(prefix='codes_', suffix='.json', dir=codes_dir)
                with os.fdopen(fd, 'w', encoding='utf-8') as f:
                    json.dump(data, f, ensure_ascii=False, indent=2)
                os.replace(tmp_path, self.codes_file)
                return True
            except Exception as e:
                logger.exception("保存文件时发生错误: %s", e)
                return False
